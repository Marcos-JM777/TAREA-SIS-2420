...
class Ventas(tk.Frame):
    ...
    def widgets(self):
        ...
        boton_agregar = tk.Button(lblframe1, text="Agregar Articulo", bg="#3498db", fg="white", font="sans 16 bold", command=self.registrar)
        ...
        ruta=self.rutas(r"icono/pagar.png")
        imagen_pil = Image.open(ruta)
        imagen_resize = imagen_pil.resize((30,30))
        imagen_tk= ImageTk.PhotoImage(imagen_resize)
        
        boton_pagar = tk.Button(lblframe1, text="Pagar", bg="#2eec71",fg="white", font="sans 16 bold", command=self.abrir_ventana_pago)
        ...
        ruta=self.rutas(r"icono/facturas.png")
        imagen_pil = Image.open(ruta)
        imagen_resize = imagen_pil.resize((30,30))
        imagen_tk= ImageTk.PhotoImage(imagen_resize)

        boton_ver_facturas = tk.Button(lblframe1, text="Ver facturas", bg="#34495e",fg="white", font="sans 16 bold", command=self.abrir_ventana_factura)
        boton_ver_facturas.config(image=imagen_tk, compound=LEFT, padx=10)
        boton_ver_facturas.image = imagen_tk
        boton_ver_facturas.place(x=750, y=10, width=240, height=50)

        self.label_suma_total = tk.Label(frame2, text="Total a pagar: BOB 0", bg="#C6D9E3", font="sans 25 bold")
    ...
    def actualizar_total(self):
        ...
        for child in self.tree.get_children():
            ...
            total += subtotal
        self.label_suma_total.config(text=f"Total a pagra: BOB {total:.0f}")
    ...
    def registrar(self):
        ...
        if producto and precio and cantidad:
            ...
            messagebox.showerror("Error", "Debe completar todos los camopos")
    ...
    def abrir_ventana_pago(self):
        ...
        label_total = tk.Label(ventana_pago, bg="#C6D9E3", text=f"Total a pagar: BOB {self.obtener_total():.0f}", font="sans 18 bold")
        ...
        def calcular_cambio():
            try:
                ...
                if cambio < 0:
                    ...
                    return
                label_cambio.config(text=f"Vuelto: BOB {cambio:.0f}")
            except ValueError:
    ...
    def pagar(self, ventana_pago, entry_cantidad_pagada, label_cambio):
        try:
            ...
            cambio = cantidad_pagada - total
            if cambio <0:
                messagebox.showerror("Error", " La cantidad pagad es insuficiente")
                return
            ...
            try:
                ...
                self.label_suma_total.config(text="Total a pagra: BOB 0")
    ...
    def generar_factura_pdf(self, productos, total, factura_numero, fecha):
        archivo_pdf = f"facturas/facturas_{factura_numero}.pdf"
        ...
        c.setFont("Helvetica-Bold", 16)
        c.drawString(100, height - 250, f"Total a pagar: BOB {total:.0f}")
        ...
        messagebox.showinfo("Factura generada", f"La factura #{factura_numero} ha sido creada exitosamenete")
    ...
    def abrir_ventana_factura(self):
        ...
        tree_facturas = ttk.Treeview(treeFrame, columns=("ID", "Factura", "Producto", "Precio", "Cantidad", "Subtotal"), show="headings", height=10, yscrollcommand=scrol_y.set,xscrollcommand=scrol_x.set)
