...
class Inventario(tk.Frame):
    ...
    def widgets(self):
        ...
        ruta=self.rutas(r"icono/producto.png")
        imagen_pil = Image.open(ruta)
        imagen_resize = imagen_pil.resize((30,30))
        imagen_tk= ImageTk.PhotoImage(imagen_resize)

        boton_agregar = tk.Button(labelframe, text="Ingresar", font="sans 14 bold", bg="#3498db", command=self.registrar)
        ...
        boton_editar = tk.Button(labelframe, text="Editar producto", font="sans 14 bold", bg="#2eec71", command=self.editar_producto)
        ...
        btn_actualizar = Button(frame2, text="Actualizar inventario", font="sans 14 bold",bg="#34495e", fg="black", command=self.actualizar_inventario)
    ...
    def mostrar(self):
        ...
        for elem in result:
            try:
                precio_bob = "{:,.0f} BOB".format(float(elem[3])) if elem[3] else ""
                costo_bob = "{:,.0f} BOB".format(float(elem[4])) if elem[4] else ""
            except ValueError:
                precio_bob = elem[3]
                costo_bob = elem[4]
            self.tre.insert("", 0, text=elem[0], values=(elem[0], elem[1], elem[2], precio_bob, costo_bob, elem[5]))
    ...
    def editar_producto(self):
        ...
        def guardar_cambios():
            ...
            try:
                ...
                messagebox.showwarning("Guardar cambios", " Ingrese valores numericos validos")
                return
